...
<main class="w-full flex-grow p-6">
    <h1 class="text-3xl text-black pb-6">{{ __('Discos Duros') }}</h1>
    
    <a href="{{ route('hdds.create') }}" class="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded">
                ...
                @foreach($hdds as $item)            
                        <tr>
                            ...
                            <td class="flex justify-center content-center">
                                <a href="{{route('hdds.edit',['hdd' => $item])}}" class="bg-yellow-400 hover:bg-yellow-300 text-white font-bold py-2 px-4 border-b-4 border-yellow-700 hover:border-yellow-500 rounded">
                                    <i class="fas fa-pencil-alt xl:mr-2"></i><samp class="hidden xl:inline">{{__('Editar')}}</samp>
                                </a>
                                <form action="{{route('hdds.destroy',['hdd' => $item])}}" method="POST" class="form-eliminar inline px-1">
                                    @method('DELETE')
                                    @csrf
