...
<div class="w-5/6 mx-auto">
    ...
    <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" action="{{ $route }}" method="POST">
        ...
        <div class="mb-3">
            <label class="block text-gray-700 text-sm font-bold mb-2 uppercase">{{__('Nombre de la categoría')}}<samp class="text-red-500">*</samp></label>
            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="nombre" name="nombre" type="text" value="{{old('nombre') ?? $categoria->nombre}}">            
            @error('nombre')
        ...
        <div class="mb-3">
            ...
            <textarea class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="descripcion" name="descripcion" type="text">{{old('descripcion') ?? $categoria->descripcion}}</textarea>             
            @error('descripcion')
