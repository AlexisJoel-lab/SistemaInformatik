...
<main class="w-full flex-grow p-6">
    <h1 class="text-3xl text-black pb-6">{{ __('Cámaras Internas') }}</h1>
    
    <a href="{{ route('camInternas.create') }}" class="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded">
    ...
    <div class="w-full mt-12">
        <div class="relative flex">
            <p class="relative text-xl pb-3 flex items-center">
                <i class="fas fa-list mr-3"></i> {{ __('Listado de productos') }}
            </p>
                ...
                @foreach($camInternas as $item)            
                        <tr>
                            ...
                            <td class="flex justify-center content-center">
                                <a href="{{route('camInternas.edit',['camInterna' => $item])}}" class="bg-yellow-400 hover:bg-yellow-300 text-white font-bold py-2 px-4 border-b-4 border-yellow-700 hover:border-yellow-500 rounded">
                                    <i class="fas fa-pencil-alt xl:mr-2"></i><samp class="hidden xl:inline">{{__('Editar')}}</samp>
                                </a>
                                <form action="{{route('camInternas.destroy',['camInterna' => $item])}}" method="POST" class="form-eliminar inline px-1">
                                    @method('DELETE')
                                    @csrf
